...
namespace azuretest6.EntityFrameworkCore.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly azuretest6DbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = azuretest6Consts.MultiTenancyEnabled ? null : (int?)MultiTenancyConsts.DefaultTenantId;
        ...
        public DefaultLanguagesCreator(azuretest6DbContext context)
        {
            _context = context;
        }
